...
class Collection extends CollectionBase
{
     ...
     */
    public function page(int $page, ?int $page_size = null): CollectionPage
    {
        return new CollectionPage($this->apiAdapter->request(
            $this->request->setPage($page, $page_size)
        ));
    }
    ...
    public function get(int $position)
    {
        ...
        return $this->pageBuffer->get($position % $this->request->getPageSize());
    }
    ...
    protected function isBuffered(int $position): bool
    {
        ...
        $first_position = $this->request->getPageSize() * $this->pageBuffer->getPage();
    ...
    {
        $this->bufferPage(floor($position / $this->request->getPageSize()));
    }
    ...
    protected function bufferPage(int $page): void
    {
        if (is_null($this->pageBuffer) || $this->pageBuffer->getPage() !== $page) {
            $this->pageBuffer = $this->page($page);
        }
    }
